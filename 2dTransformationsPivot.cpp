...
void translationPivot(int (&points)[10][2],int n, int px, int py);
void rotation(int (&points)[10][2],int n, int px, int py);
void scaling(int (&points)[10][2],int n, int px, int py);
...
int main()
{
    ...
    cin>>px>>py;
  //  px+=xo;
   // py=yo-py;  //to adjust pivot point to central origin instead of traditional computer top left origin
    circle(px+xo,yo-py,5);   //to make sure pivot point is clearly visible
    system("cls");
        ...
        switch(ch)
          {
            case 1:
                    ...
                    break;
            case 2:
                    rotation(points,n,px,py);
                    break;
            case 3:
                    scaling(points,n,px,py);
                    break;
            default:
                    cout<<"Invalid Choice!";
          }
...
void drawpolygon(int points[][2],int n)
 ...
 void displaypoints(int points[][2],int n)
  ...
  void translationPivot(int (&points)[10][2],int n,int px,int py)
  {
    for(int i=0;i<n;i++)
      {
        points[i][0]=points[i][0]+px;
        points[i][1]=points[i][1]+py;
      }
  }
  ...
  void rotation(int (&points)[10][2],int n, int px, int py)
  {
   ...
   translationPivot(points,n,-px,-py); //move pivot to origin
    if(dir=='A')
      {
        for(int i=0;i<n;i++)
          {
    ...
    cout<<"\n\nPoints after transformation:\n";
    translationPivot(points,n,px,py);  //restore back to original pivot
    displaypoints(points,n);
    setcolor(GREEN);
    drawpolygon(points,n);
  }
  ...
  void scaling(int (&points)[10][2],int n, int px, int py)
  {
    ...
    cin>>sy;
    translationPivot(points,n,-px,-py);
    for(int i=0;i<n;i++)
      {
